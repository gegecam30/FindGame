...
public class GameManager {
    ...
    public boolean joinGame(Player player, String arenaName) {
        ...
        // Buscar arena
        Arena arena = (arenaName != null) ? plugin.getArenaManager().getArena(arenaName) : plugin.getArenaManager().getRandomArena();
        ...
        startRound(player, arena);
        return true;
    }

    /**
     * Inicia una ronda (Spawn, Teleport, Timer).
     */
    private void startRound(Player player, Arena arena) {
        ...
        String title = plugin.getConfigManager().getMessageWithPlaceholders("level-title", "level", String.valueOf(level));
        String subtitle = plugin.getConfigManager().getMessage("level-subtitle"); 
        player.sendTitle(title, subtitle, 10, 40, 10);

        // Mensaje de inicio
        player.sendMessage(plugin.getConfigManager().getMessageWithPlaceholders("game-start", "level", String.valueOf(level)));
    ...
    private void handleVictory(Player player) {
        ...
        // Subir de Nivel
        int nextLevel = playerLevels.getOrDefault(player.getUniqueId(), 1) + 1;
        playerLevels.put(player.getUniqueId(), nextLevel);
        plugin.getStatsManager().updateMaxLevel(player.getUniqueId(), nextLevel);

        // Siguiente ronda con delay
        Bukkit.getScheduler().runTaskLater(plugin, () -> {
            if (isPlaying(player)) {
                Arena nextArena = plugin.getArenaManager().getRandomArena();
                if (nextArena != null) startRound(player, nextArena);
            }
        }, 40L);
    }
    ...
    private void handleDefeat(Player player, Entity innocentEntity) {
        ...
        if (currentLives <= 0) {
            ...
            // Pierde una vida
            player.sendMessage(plugin.getConfigManager().getMessageWithPlaceholders("game-life-lost", "lives", String.valueOf(currentLives)));
            player.playSound(player.getLocation(), Sound.ENTITY_ITEM_BREAK, 1f, 1f);

            // Si falló por tiempo (sin golpear), reiniciamos ronda
            if (innocentEntity == null) {
                stopTimer(player);
                plugin.getNpcManager().cleanup(player);
                Bukkit.getScheduler().runTaskLater(plugin, () -> {
                    if (isPlaying(player)) {
                        Arena a = plugin.getArenaManager().getRandomArena();
                        if (a != null) startRound(player, a);
                    }
                }, 20L);
            }
        }
    ...
    private void startTimer(Player player, int seconds) {
        ...
        BukkitTask task = Bukkit.getScheduler().runTaskTimer(plugin, new Runnable() {
            ...
            public void run() {
                ...
                if (currentBar != null) {
                    String currentTitle = ColorUtil.colorize(bossFormat.replace("{time}", String.valueOf((int)timeLeft)));
